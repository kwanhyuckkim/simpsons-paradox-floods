...
**Status**: research code accompanying paper preprint (DOI: TBD)
**License**: MIT
**Python**: 3.11+
**Documentation**: https://<user>.github.io/ungauged-flood-bhm/
...
- **Approach** — 4-stage stacking pipeline. (1) RFE with stability-weighted importance, (2) Bayesian hierarchical model (Bambi + blackjax NUTS) with `Natural_Managed × HUC02 × Köppen × area-bin` random
slopes, (3) GPyTorch spatial GP on residuals per dynamic variable, (4) Quantile RF stacking with BHM β-features + GP features.
- **Result** — Adjusted R² 0.81 / SMAPE 18.4 on held-out gauges (vs Paper 1 baseline 0.77 / 20.5); PICP 0.90 at α=0.10; recovers true positive ISA elasticity across 87% of basin clusters.

> **Figure 1.** *Hierarchical Bayesian model resolves Simpson's paradox. Top panel: pooled OLS shows negative ISA→flood slope. Bottom panel: BHM group-specific posterior medians reveal positive elasticity 
in 87% of basin clusters.*
> (placeholder — see `docs/figures/killer_figure.png` after first `make figures`)
...
Raw data is not included in git — see `docs/data.md` for one-command download.

## Reproducibility

Full reproduction via `make reproduce` (~4 GPU-hours, ~50 GB data, ~16 GB RAM). Step-by-step instructions in `docs/reproduce.md`.
...
This work was conducted at UMass Amherst Department of Civil and Environmental Engineering. Computation supported by the Unity HPC cluster. Funded by [grant numbers].

Built on `bambi`, `blackjax`, `bayeux`, `gpytorch`, `quantile-forest`, `arviz`, `pymc`. Builds on Kratzert et al. 2019 (NeuralHydrology) and Sadler et al. 2022 (mHM-PUB).

## Contact

Kwan-Hyuck Kim · kwanhyuckkim@umass.edu
